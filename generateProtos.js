...
async function generateProtoFiles(client) {
  ...
  for (const interfaceName of interfaces) {
    ...
    await generateProtoFile(interfaceName, implementations, protoDir);
  }

  await generateCommonProtoFiles(protoDir);
}

async function generateProtoFile(interfaceName, implementations, protoDir) {
...
async function main() {
  ...
  const client = await createReflectionClient(endpoint, useTLS === 'TLS');
